...
def get_rich_poor_ratio(model):
    ...
    rich_count = sum(isinstance(agent, Person) and agent.savings > model.rich_threshold for agent in model.schedule.agents)
    poor_count = sum(isinstance(agent, Person) and agent.loans > 10 for agent in model.schedule.agents)
