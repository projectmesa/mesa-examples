...
def custom_batch_run(
    model_cls, parameters, iterations, max_steps, model_reporters, agent_reporters
):
    results = []
    for params in (
        dict(zip(parameters, x)) for x in itertools.product(*parameters.values())
    ):
        for i in range(iterations):
            ...
            for step in range(max_steps):
                model.step()
            model_data = {
                var: reporter(model) for var, reporter in model_reporters.items()
            }
            agent_data = [
                {var: getattr(agent, var) for var in agent_reporters}
