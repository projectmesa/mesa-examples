...
class Cholera(mesa.Model):
    def __init__(
        ...
        self.grid = mesa.experimental.cell_space.VoronoiGrid(centroids_coordinates=pumps_location, capacity=int(self.susceptible + 1), random=self.random)
        
        print(self.grid.all_cells)
        for population, cell, contaminated in zip(cell_population, list(self.grid.all_cells), is_pump_contaminated):
            pump_state = INFECTIOUS if contaminated else SUSCEPTIBLE
            self.infected_pumps += pump_state
            pump = Pump(
                self.next_id(), self, pump_state, 
